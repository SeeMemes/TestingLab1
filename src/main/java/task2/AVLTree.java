...
public class AVLTree extends BinarySearchTreeRecursive {
    ...
    public ArrayList<Integer> printTree (Node node, ArrayList<Integer> list){
        list.add(node.data);
        if (node.left != null) printTree(node.left, list);
        if (node.right != null) printTree(node.right, list);
        return list;
    }
